...
class _BoundedRandomStep:
    def __init__(self, lower, upper, stepsize=1.0, seed=15):
        self.lower = _np.asarray(lower, dtype=float)
        self.upper = _np.asarray(upper, dtype=float)
    ...
    def __call__(self, params):
        displacement = self.rng.uniform(
            -self.stepsize,
            self.stepsize,
            size=_np.shape(params),
        )
        return _np.clip(params + displacement, self.lower, self.upper)
...
def DFFT(df, column: str, numShards: int):
    ...
    indexed_rdd = (
        df.select(column)
          ...
          .zipWithIndex()
          .map(lambda value_index: (value_index[1], value_index[0]))
          .cache()
    )
    N = indexed_rdd.count()
    P = numShards
    ...
    if rem > 0:
        ...
        indexed_rdd = indexed_rdd.filter(lambda index_value: index_value[0] < N - rem)
    N_effective = N - rem
    M = N_effective // P

    subffts = (
        indexed_rdd
           .map(lambda index_value: (
               index_value[0] % P,
    ...
    twiddled = subffts.map(lambda triple: (
        triple[0],
        triple[1],
        triple[2] * cmath.exp(-2j * math.pi * (triple[0]//M)*(triple[0]%M) / N_effective)
    ))
...
def sampler(q: int, p: int,
    ...
    lp = log_prior(theta_map, 0, 1, Last_ARMA, TFI_term, n_groups)
    ...
    for i in range(n_samples):
        ...
        lp_prop = log_prior(theta_prop, 0, 1, Last_ARMA, TFI_term, n_groups)
    ...
    return draws[burn_in:], logp_trace[burn_in:], accepts[burn_in:]
...
def mapper(
    ...
    lower = _np.full(n_params, -process_limit)
    upper = _np.full(n_params, process_limit)
    if TFI:
        lower[-3:] = -30.0
        upper[-3:] = 30.0
    else:
    ...
    theta0 = _np.clip(theta0, lower, upper)
    ...
    with warnings.catch_warnings():
        ...
        res = basinhopping(obj, 
                           ...
                           niter=100, 
                           stepsize=1, 
                           take_step=_BoundedRandomStep(lower, upper, seed=shard_seed),
                           minimizer_kwargs={
                               "method": "trust-constr", 
                               "jac":jcb, 
                               "hess":hs, 
    ...
    if not res.success:
        raise RuntimeError(f"MAP optimization failed for shard {shard_id}: {res.message}")

    theta_map = _np.asarray(res.x, dtype=float)
    if _np.any(theta_map < lower) or _np.any(theta_map > upper):
        raise RuntimeError(f"MAP optimization returned infeasible parameters for shard {shard_id}")
...
if __name__ == "__main__":
    ...
    P = 16  # DFFT execution partitions
    G = 16  # Whittle posterior groups
    #spark.conf.set("spark.sql.shuffle.partitions", str(G))
    # 1) DFFT gives full FFT coefficients X[k]
    dfft_df = DFFT(raw_df, "y", numShards=P).cache()
    N_full = dfft_df.count()   # 这一步会跑一次，但之后复用 cache
    n_freq = int(np.floor((N_full - 1) / 2))
    if G < 1 or G > n_freq:
        raise ValueError(f"G must satisfy 1 <= G <= {n_freq}, but got G={G}")
    
    # 2) Build periodogram exactly matching p_gram(fft(data))
    periodogram_df = (
        dfft_df
        .filter((F.col("k") >= 1) & (F.col("k") <= F.lit(n_freq)))
        ...
        .withColumn(
            "shard_id",
            F.pmod(F.col("k") - F.lit(1), F.lit(G))   # zero-based systematic split
        )
